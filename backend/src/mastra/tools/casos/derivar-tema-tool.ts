...
export const derivarTemaTool = createTool({
  id: "derivar-tema",
  description: `Marcá que el usuario trajo un asunto de OTRA área legal, además del que venís atendiendo. Lo que decide es el ÁREA, no que el asunto sea nuevo: si cae en la tuya seguí atendiéndolo vos sin marcar, aunque toque otra subcategoría y aunque sea un asunto separado del primero (otro contrato, otro inmueble, otro empleador, otro vehículo). Pasá el tema en las palabras del usuario: la clasificación no la hacés vos. Ante la duda de si el asunto cae fuera de tu área, marcá igual — si termina siendo tuyo no pasa nada, pero el que no marcás se pierde como caso.`,
