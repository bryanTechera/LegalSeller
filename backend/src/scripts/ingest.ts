 ...
 *
 * Uso: pnpm ingest <archivo.txt> --title "Ley N° 17.250 — Defensa del Consumidor (Uruguay)"
 *
...
async function main(): Promise<number> {
  ...
  if (!filePath || !title) {
    logger.error("Uso: pnpm ingest <archivo.txt> --title \"<título del documento>\"");
    return 1;
  }
  ...
  const { rows } = await pool.query<{ id: string }>(
    `INSERT INTO "Document" ("id", "title", "sourceKey", "status", "createdAt", "updatedAt")
     VALUES (gen_random_uuid()::text, $1, $2, 'PROCESSING'::"ProcessingStatus", now(), now())
     ON CONFLICT ("title") DO UPDATE SET "sourceKey" = $2, "status" = 'PROCESSING'::"ProcessingStatus", "updatedAt" = now()
     RETURNING "id"`,
    [title, filePath],
  );
