...
13. **Las dudas de dominio se derivan a los expertos legales.** El equipo técnico cuenta con el apoyo de un equipo de expertos legales. Ante ambigüedad o falta de conocimiento **no técnico** durante la implementación (qué debe responder un agente ante cierto caso, criterios o plazos legales, qué documentos integran el corpus de una subcategoría, tratamiento de casos sensibles), la duda no se resuelve asumiendo ni inventando: se formula concreta y se deriva al equipo de expertos. Mientras tanto se implementa lo no ambiguo y la duda se registra en un archivo **enviable** en `docs/preguntas-legales/YYYY-MM-DD-<tema>.md`, redactado para los abogados (auto-contenido, con cita del pasaje dudoso y la pregunta concreta; sin jerga del repo) y con estado (`PENDIENTE`/`RESPONDIDA`), referenciado desde el plan correspondiente en `docs/plans/`. Ese archivo se envía tal cual al equipo legal — un solo lugar por lote de preguntas agiliza la ida y vuelta. Las respuestas de los expertos que fijen criterio se documentan para no volver a preguntar lo mismo.
