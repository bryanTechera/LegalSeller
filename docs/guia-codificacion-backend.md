...
Model stack de referencia (calibrar con evals): agentes principales → modelo lite rápido (`gemini-3.5-flash-lite`); sub-agentes expertos y generadores → tier lite; jueces de evals → el lite más barato; retrieval web (si se necesita) → `perplexity/sonar` con `tools: {}` obligatorio.
...
- **Rules vs skills**: rules = restricciones de comportamiento ("NUNCA afirmes contenido legal sin respaldo de buscar-documentos"); skills = conocimiento del dominio (criterios jurídicos, heurísticas de interpretación). Rules ordenadas por prioridad de atención (primacy/recency — lo crítico al principio o al final, nunca en el medio).
