...
Los agentes de LegalSeller corren `google/gemini-3.5-flash-lite` (`crearAgente`). **Si el primer scorer LLM-as-judge se implementa con un juez Gemini** (ej. el "lite más barato" que sugiere `guia-codificacion-backend.md § 9`, que es Gemini), el setup queda **same-family** y aparece family bias tipo libro de texto:
