...
describe("estimarCostoUsd", () => {
  ...
  it("el modelo anterior conserva su precio para el costo histórico", () => {
    expect(estimarCostoUsd("google/gemini-3-flash", 1_000_000, 1_000_000)).toBeCloseTo(3.5, 5);
  });

  it("acepta el id de modelo sin el prefijo del proveedor", () => {
    expect(estimarCostoUsd("gemini-3.5-flash-lite", 1_000_000, 0)).toBeCloseTo(0.3, 5);
  });
  ...
  it("cero tokens con modelo conocido cuesta cero", () => {
    expect(estimarCostoUsd("google/gemini-3.5-flash-lite", 0, 0)).toBe(0);
  });
});
